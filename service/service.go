...
func Run(ctx context.Context, cfg *config.Config, serviceList *ExternalServiceList, buildTime, gitCommit, version string, svcErrors chan error) (svc *Service, err error) {
	...
	dc, err := getCopier(ctx, cfg, serviceList, svc)
	if err != nil {
		return nil, err
	}
...
func getCopier(ctx context.Context, cfg *config.Config, serviceList *ExternalServiceList, svc *Service) (file.Copier, error) {
	...
	return file.NewCopier(publicClient, privateClient, svc.FilesClient), nil
}
