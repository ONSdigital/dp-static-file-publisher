...
func (e *ExternalServiceList) GetS3Clients(ctx context.Context, cfg *config.Config) (s3Private, s3Public file.S3Client, err error) {
	...
	e.S3Public = true
	s3Private, err = e.Init.DoGetS3Client(ctx, cfg.AwsRegion, cfg.PrivateBucketName)
	if err != nil {
		return nil, nil, err
	}
	e.S3Private = true
...
func (e *Init) DoGetS3Client(ctx context.Context, awsRegion, bucketName string) (file.S3Client, error) {
	...
	// If running locally using localstack, `BaseEndpoint` needs to be defined and `UsePathStyle`
